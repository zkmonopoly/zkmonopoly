...
export function Player(props: PlayerProps) {
  ...
  useEffect(() => {
    ...
    console.log(
      `Player ${props.playerIndex} movement sequence: ${sequence}`
    );
  ...
  useBeforeRender((scene) => {
    if (scene && playerMeshRef.current) {
      if (!playerMeshRef.current.position.equals(positionRef.current)) {
        playerMeshRef.current.position = Vector3.Lerp(
          playerMeshRef.current.position,
          positionRef.current,
          0.1
        );
        if (
          playerMeshRef.current.position.equalsWithEpsilon(
            positionRef.current,
            0.1
          )
        ) {
        ...
        if (movement) {
          ...
          positionRef.current = NodePositions[movement].add(
            new Vector3(
              ...
              0.5,
              PlayerPositions[props.playerIndex].z
            )
          );
        }
      }
  ...
  useEffect(() => {
    console.log(
      `Player ${props.playerIndex} position: ${props.playerState}`
    );
    ...
    if (props.playerIndex === 0) {
      ...
      playerMeshRef.current = playerTexture.loadedMeshes[0].clone(
    ...
    const playerMaterial = new StandardMaterial(
      `player${props.playerIndex}Material`,
            scene!
    );
